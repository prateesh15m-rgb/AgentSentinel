# agentops/run_full_eval_for_version.py
...
def _trace_event_from_aut_response(
    ...
    """
    # tool_calls is a list of ToolCall dataclasses in QnAAUTClient;
    # convert to plain dicts for JSONL store.
    tool_calls_dicts = []
    for tc in getattr(aut_resp, "tool_calls", []) or []:
        # supports both dataclass (with __dict__) and plain dict
...
def main():
    ...
    # AUTClient wrapper around the legacy Q&A agent
    aut_client = QnAAUTClient(version_id=version_id)

    scores = []
    print(
        f"Running full evaluation on {len(golden)} test cases for version '{version_id}'...\n"
    )
    ...
    for row in golden:
        ...
        # Run the chosen agent version via AUTClient
        ...
        # Evaluate using existing eval pipeline
    ...
    print("===================================")
    print(f" Full Evaluation Complete for {version_id}")
    print("===================================")
